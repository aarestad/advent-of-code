...
def solve_part_1(directions):
    ...
    print("part 1: hq is %s blocks away at (%s, %s)" % (abs(curr_x) + abs(curr_y), curr_x, curr_y))
    ...
    for direction in directions:
        ...
        while travel_distance > 0:
            ...
            if (curr_x, curr_y) in currently_visited:
                print("part 2: hq is %s blocks away at (%s, %s)" % (abs(curr_x) + abs(curr_y), curr_x, curr_y))
                return
