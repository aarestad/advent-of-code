...
def validate_record(record: dict):
    try:
        return (
            1920 <= int(record["byr"]) <= 2002
            and 2010 <= int(record["iyr"]) <= 2020
            and 2020 <= int(record["eyr"]) <= 2030
            and validate_height(record["hgt"])
            and re.match("^#[0-9a-f]{6}$", record["hcl"])
            and record["ecl"] in valid_eye_colors
            and re.match(r"^\d{9}$", record["pid"])
        )
