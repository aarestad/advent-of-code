...
if __name__ == '__main__':
    ...
    for i in count(1):
        if i % 10000 == 0: print('step {}'.format(i))
        ...
        if not steps_for_x and all(p.pos.x == initial_state[p.name].pos.x and p.vel.x == initial_state[p.name].vel.x for p in planets):
            steps_for_x = i
            print('steps for x: {}'.format(steps_for_x))
        
        if not steps_for_y and all(p.pos.y == initial_state[p.name].pos.y and p.vel.y == initial_state[p.name].vel.y for p in planets):
            steps_for_y = i
            print('steps for y: {}'.format(steps_for_y))
            
        if not steps_for_z and all(p.pos.z == initial_state[p.name].pos.z and p.vel.z == initial_state[p.name].vel.z for p in planets):
            steps_for_z = i
